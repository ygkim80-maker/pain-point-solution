...
2. `.env` 파일 생성 (`.env.example` 참고)
   ...
   - `SIGNUP_CODE` (강력 권장): 회원가입 시 요구할 코드. 이 앱은 공개 URL로 배포되므로, 설정하지 않으면 인터넷의 누구나 가입해서 내부 글을 볼 수 있습니다. 직원에게만 공유하세요. 비워두면 코드 없이 누구나 가입 가능(비권장, 초기 테스트용).
...
- 비밀번호는 bcrypt로 해시 저장, 세션은 서명된 JWT를 httpOnly 쿠키로 발급
- 회원가입은 `SIGNUP_CODE` 환경변수로 게이트 — **반드시 설정**하고 직원에게만 공유하세요
- 로그인 5회 연속 실패 시 15분간 잠금 (무차별 대입 방어)
- 비밀번호 최소 8자
- 관리자가 발급하는 임시 비밀번호는 암호학적으로 안전한 난수(Node `crypto`)로 생성
- 모든 Server Action은 개별적으로 인증/권한 검사를 수행 (페이지 렌더링 여부와 무관하게 안전)
- 기본 보안 헤더(CSP, X-Frame-Options, Referrer-Policy 등) 적용 (`next.config.ts`)
- 사용자 콘텐츠는 전부 React JSX를 통해 렌더링되어 자동으로 이스케이프됨 (`dangerouslySetInnerHTML` 미사용)
